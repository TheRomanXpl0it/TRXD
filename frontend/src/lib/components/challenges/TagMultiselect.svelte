		...
		<Popover.Content class="w-80 p-3">
			<div class="flex flex-col gap-3">
				<input
					...
					class="border-input placeholder:text-muted-foreground focus-visible:ring-ring flex h-9 w-full rounded-md border bg-transparent px-3 py-1
				...
				{#if canCreate}
					<button
						type="button"
						class="hover:bg-accent flex w-full items-center gap-2 rounded border border-dashed px-2 py-1.5 text-left"
				...
				<div class="max-h-60 overflow-auto pr-1">
					...
					{:else}
						{#each filtered as v, i (i)}
							<button
								type="button"
								class={`hover:bg-accent flex w-full items-center gap-2 rounded px-2 py-1.5 text-left ${i === active ? 'bg-accent/60' : ''}`}
								onclick={() => toggle(v)}
