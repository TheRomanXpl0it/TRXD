<script lang="ts">
  ...
  // unified time getter
  const t = (s: any) =>
    new Date(s?.createdAt ?? s?.time ?? s?.timestamp ?? Date.now()).getTime();
  ...
  function goTeam(id: string | number | undefined, ev?: Event) {
    if (!id) return;
...
<Sheet.Root bind:open>
  <Sheet.Content side="left" class="sm:max-w-[640px] px-5">
    <Sheet.Header>
