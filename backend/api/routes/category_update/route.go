...
func Route(c *fiber.Ctx) error {
	...
	if data.Name == "" || (data.NewName == "" && data.NewIcon == "") {
		return utils.Error(c, fiber.StatusBadRequest, consts.MissingRequiredFields)
	}
	if len(data.Name) > consts.MaxCategoryLength {
		return utils.Error(c, fiber.StatusBadRequest, consts.LongCategory)
	}
	if len(data.NewName) > consts.MaxCategoryLength {
		return utils.Error(c, fiber.StatusBadRequest, consts.LongCategory)
	}
	if len(data.NewIcon) > consts.MaxIconLength {
	...
	if data.NewName != "" && data.Name != data.NewName {
		...
		category, err := category_create.CreateCategory(c.Context(), data.NewName, icon)
		if err != nil {
		...
		err = category_delete.DeleteCategory(c.Context(), data.Name)
		if err != nil {
